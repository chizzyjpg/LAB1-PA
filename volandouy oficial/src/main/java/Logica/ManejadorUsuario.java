...
/**
 * ManejadorUsuario: clase de utilidad para convertir entre DTOs de usuario y entidades de dominio.
 */
public final class ManejadorUsuario {

  private ManejadorUsuario() {
  } // Constructor privado: evita instanciar la clase (solo utilidad estática).

  //// DTO ES DATAT TRANSFER OBJET ---- OBJETO HECHO DATA TYPE PARA PASAR A CAPA
  //// DE PRESENTACION
  ...
  /**
   * Convierte un DTO de usuario a su entidad de dominio correspondiente.
   * 
   */
  public static Usuario toEntity(DataUsuario dto) { // Convierte un DTO (capa de transporte) a
                                                    // Entidad de dominio.
    Objects.requireNonNull(dto, "DataUsuario no puede ser null"); // Valida que dto no sea null; si
                                                                  // lo es, lanza
    ...
    throw new IllegalArgumentException("Tipo de DataUsuario no soportado: " + dto.getClass()); // Si
                                                                                               // llega
                                                                                               // aquí,
                                                                                               // el
                                                                                               // tipo
  ...
  // =========================
  /**
   * Convierte una entidad de dominio de usuario a su DTO correspondiente.
   * 
   */
  public static DataUsuario toDTO(Usuario u) { // Convierte una entidad de dominio a su DTO
                                               // correspondiente.
    ...
    if (u instanceof Cliente c) { // Si la entidad es Cliente, la captura en c (ya casteada).
      return new DataCliente( // Crea el DTO DataCliente a partir de la entidad Cliente.
          /* nombre */ c.getNombre(), /* nickname */ c.getNickname(), /* email */ c.getEmail(),
          /* contra */ c.getContrasenia(), /* apellido */ c.getApellido(),
          /* fechaNac */ copy(c.getFechaNac()), // Copia defensiva del Date.
          /* nacional */ c.getNacionalidad(), /* tipoDoc */ c.getTipoDocumento(),
      ...
      return new DataAerolinea( // Crea el DTO DataAerolinea desde la entidad.
          /* nombre */ a.getNombre(), /* nickname */ a.getNickname(), /* email */ a.getEmail(),
          /* contra */ a.getContrasenia(), /* descripcion */ a.getDescGeneral(), // Nota: nombres de
                                                                                 // getters
                                                                                 // distintos a los
                                                                                 // del DTO.
    ...
    throw new IllegalArgumentException("Tipo de Usuario no soportado: " + u.getClass()); // Si la
                                                                                         // entidad
                                                                                         // no es un
                                                                                         // tipo
                                                                                         // soportado,
  ...
  // =========================
  /**
   * Convierte una lista de DTOs de usuario a una lista de entidades de dominio.
   * 
   */
  public static List<Usuario> toEntities(List<? extends DataUsuario> dtos) { // Convierte una lista
                                                                             // de DTOs a entidades.
  ...
  /**
   * Convierte una lista de entidades de dominio de usuario a una lista de DTOs.
   * 
   */
  public static List<DataUsuario> toDTOs(List<? extends Usuario> users) {// Convierte una lista de
                                                                         // entidades a DTOs.
    return users.stream().map(ManejadorUsuario::toDTO).collect(Collectors.toList());
    // return user.stream() Stream sobre la lista de usuarios… //
    // .map(ManejadorUsuario::toDTO mapea cada entidad a su DTO… //
    // .collect(Collectors.toList y colecta en una List.
  }
  ...
  private static Date copy(Date d) { // Helper para evitar exponer mutabilidad de Date.
    return (d == null) ? null : new Date(d.getTime()); // Si d es null, retorna null; si no, crea un
                                                       // Date nuevo con el mismo epoch.
  }
