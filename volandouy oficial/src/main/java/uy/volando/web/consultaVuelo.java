...
/**
 * Servlet para la consulta de vuelos, rutas y reservas.
 */
    ...
    if (accion == null || accion.isBlank()) {
      ...
      if (nicknameAerolinea != null && !nicknameAerolinea.isBlank()) {
        ...
        List<DataRuta> rutasConfirmadas = rutas.stream()
            .filter(r -> r.getEstado() == EstadoRuta.CONFIRMADA).toList();
      ...
      if (nicknameAerolinea != null && !nicknameAerolinea.isBlank() && nombreRuta != null
        ...
        System.out.println(
            "[consultaVuelo] Detalle vuelo: " + (vuelo != null ? vuelo.getNombre() : "null"));
        ...
        if (usuario != null) {
          ...
          System.out
              .println("[consultaVuelo] esAerolinea: " + esAerolinea + ", esCliente: " + esCliente);
          System.out.println("[consultaVuelo] Nickname aerolínea de la ruta: "
              + (vuelo.getDRuta() != null ? vuelo.getDRuta().getNicknameAerolinea() : "null"));
          ...
          if (esAerolinea && vuelo.getDRuta() != null && usuarioNickname != null
            ...
            List<DataReserva> reservas = sistema.listarReservas(nicknameAerolinea, nombreRuta,
                codigoVuelo);
            System.out.println("[consultaVuelo] Aerolínea dueña, reservasVuelo size: "
                + (reservas != null ? reservas.size() : "null"));
            ...
            List<DataReserva> reservas = sistema.listarReservas(nicknameAerolinea, nombreRuta,
                codigoVuelo);
            System.out.println("[consultaVuelo] Cliente, reservas size: "
                + (reservas != null ? reservas.size() : "null"));
            for (DataReserva r : reservas) {
              if (r.getNickCliente() != null
                  && usuarioNickname.equals(r.getNickCliente().getNickname())) {
                System.out
                    .println("[consultaVuelo] Reserva encontrada para cliente: " + usuarioNickname);
        ...
        if (idReserva != null && !idReserva.isBlank()) {
          try {
            ...
            DataReserva detalleReserva = sistema.buscarReserva(nicknameAerolinea, nombreRuta,
                codigoVuelo, idRes);
            System.out.println("[consultaVuelo] Detalle reserva id: " + idRes + " encontrada: "
                + (detalleReserva != null));
