...
/**
 * Servlet para gestionar la reserva de vuelos.
 */
...
public class reservaVuelo extends HttpServlet {
  ...
  /**
   * Maneja las solicitudes GET para mostrar la página de reserva de vuelos.
   */
  protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    if (aerolineaSel != null && !aerolineaSel.isEmpty() && rutaSel != null && !rutaSel.isEmpty()
        && vueloSel != null && !vueloSel.isEmpty()) {
      vueloDetalle = sistema.buscarVuelo(aerolineaSel, rutaSel, vueloSel);
      // Obtener paquetes vigentes del cliente si está logueado y hay vuelo
      // seleccionado
      if (usuario != null) {
        String nicknameCliente = usuario instanceof Logica.DataCliente
            ? ((Logica.DataCliente) usuario).getNickname()
            : null;
        if (nicknameCliente != null) {
          ...
          for (Logica.DataPaquete p : paquetesDisponibles) {
            System.out.println("[DEBUG] Paquete: " + p.getNombre() + ", rutasIncluidas: "
                + p.getRutasIncluidas() + ", nombreRuta: " + nombreRuta);
          }
          List<Logica.DataPaquete> paquetesFiltrados = paquetesDisponibles.stream()
              .filter(p -> p.getCantRutas() > 0).filter(p -> p.getRutasIncluidas().stream()
