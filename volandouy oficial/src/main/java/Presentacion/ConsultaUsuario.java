...
public class ConsultaUsuario extends JInternalFrame {
    ...
    public ConsultaUsuario(ISistema sistema) {
        ...
        lstReservas.setCellRenderer(new DefaultListCellRenderer() {
            @Override public Component getListCellRendererComponent(
                ...
                if (value instanceof DataReserva r) {
                	String nickCli = (r.getNickCliente() == null) ? "" : nvl(r.getNickCliente().getNickname());
        ...
        lstRutas.addMouseListener(new MouseAdapter() {
            @Override public void mouseClicked(MouseEvent e) {
                if (e.getClickCount() == 2 && lstRutas.getSelectedValue() != null) abrirConsultaRuta(lstRutas.getSelectedValue());
            }
        });
        ...
        lstReservas.addMouseListener(new MouseAdapter() {
            @Override public void mouseClicked(MouseEvent e) {
                if (e.getClickCount() == 2 && lstReservas.getSelectedValue() != null) abrirConsultaVuelo(lstReservas.getSelectedValue());
            }
        });
        ...
        lstPaquetes.addMouseListener(new MouseAdapter() {
            @Override public void mouseClicked(MouseEvent e) {
                if (e.getClickCount() == 2 && lstPaquetes.getSelectedValue() != null) abrirConsultaPaquete(lstPaquetes.getSelectedValue());
            }
        });
    ...
    private void cargarUsuariosEnCombo() {
        try {
            ...
            for (DataUsuario du : usuarios) comboUsuarios.addItem(du);
    ...
    private List<?> obtenerReservasDeCliente(DataCliente dc) {
        ...
        try {
                ...
                for (DataRuta ruta : (List<DataRuta>) sistema.listarPorAerolinea(nickAero)) {
                    ...
                    for (DataVueloEspecifico v : sistema.listarVuelos(nickAero, nombreRuta)) {
                        ...
                        for (DataReserva r : sistema.listarReservas(nickAero, nombreRuta, codVuelo)) {
                        	String nickReserva = (r.getNickCliente() == null) ? null : r.getNickCliente().getNickname();
            ...
            JOptionPane.showMessageDialog(this, "No se pudieron listar reservas del cliente:\n" + ex.getMessage(),
                    "Error", JOptionPane.ERROR_MESSAGE);
        }
        return result;
    }
    ...
    private List<?> obtenerPaquetesDeCliente(DataCliente dc) {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(this, "No se pudieron listar paquetes del cliente:\n" + ex.getMessage(),
                    "Error", JOptionPane.ERROR_MESSAGE);
        }
        return result;
    }
    ...
    private void abrirConsultaPaquete(Object dtoCompra) {
        try {
            ...
            JOptionPane.showMessageDialog(this, "No se pudo abrir Consulta de Paquete de Rutas de Vuelo:\n" + ex.getMessage(),
                    "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void agregarYMostrar(JInternalFrame jf) {
        ...
        if (desk != null) {
            desk.add(jf);
            jf.setVisible(true);
            try { jf.setSelected(true); } catch (Exception ignored) {}
        } else {
            jf.setVisible(true);
        }
    }
    ...
    private static String describirPaquete(DataPaquete p) {
        String costo = (p.getCosto() == null) ? "-" : NumberFormat.getNumberInstance().format(p.getCosto());
