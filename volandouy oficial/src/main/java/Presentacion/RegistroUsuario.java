...
/**
 * Clase que representa la interfaz gráfica para registrar un nuevo usuario
 * (Cliente o Aerolínea).
 */
public class RegistroUsuario extends JInternalFrame {
   ...
   * Runnable() { public void run() { try { RegistroUsuario frame = new
   * RegistroUsuario(sistema); frame.setVisible(true); } catch (Exception e) {
   * e.printStackTrace(); } } }); }
   */
