...
/**
 * Interfaz gráfica para la compra de paquetes por parte de clientes.
 */
public class CompraPaqueteIF extends JInternalFrame {
  ...
  /** Constructor de la interfaz de compra de paquetes.
   * 
   */
  ...
  private void construirHeader() {
    ...
    cboClientes
        .setPrototypeDisplayValue(new ClienteItem("xxxxxxxxxx", "Nombre Apellido <email@dominio>"));
    header.add(cboClientes, "growx");

    JLabel labelFechaCompra = new JLabel("Fecha de compra:");
    labelFechaCompra.setFont(new Font("Comic Sans MS", Font.PLAIN, 11));
    header.add(labelFechaCompra);
  ...
  private void construirTabla() {
    paquetesModel = new DefaultTableModel(new Object[] { "Nombre", "Descripción", "Tipo asiento",
        "Cant. rutas", "Validez (días)", "Costo" }, 0) {
      @Override
  ...
  private void cargarPaquetes() {
    try {
      ...
      for (DataPaquete p : paquetes) {
        paquetesModel.addRow(new Object[] { p.getNombre(), p.getDescripcion(),
            p.getTipoAsiento() == null ? "-" : p.getTipoAsiento().name(), p.getCantRutas(),
            p.getValidez(), formatoMoneda(p.getCosto()) });
      }
    } catch (Exception ex) {
  ...
  private void onComprar() {
    try {
      ...
      if (row < 0) {
        showWarn("Seleccione un paquete de la tabla.");
        return;
      }

      ClienteItem selCliente = (ClienteItem) cboClientes.getSelectedItem();
      if (selCliente == null) {
        showWarn("Seleccione un cliente.");
        return;
      }

      java.util.Date fechaCompra = dcFechaCompra.getDate();
      if (fechaCompra == null) {
        showWarn("Seleccione la fecha de compra.");
        return;
      }
      ...
      DataCompraPaquete dto = new DataCompraPaquete(nombrePaquete, nickCliente, fechaCompra, null, // null
                                                                                                   // =>
                                                                                                   // usa
                                                                                                   // costo
                                                                                                   // del
      ...
      JOptionPane.showMessageDialog(this, "Compra registrada con éxito.", "OK",
  ...
  private BigDecimal parseBigDecimalNullable(String raw) {
    if (raw == null || raw.isBlank()) {
      return null; // vacío = sin override      
    }

    String s = raw.trim().replace(" ", "") // quita espacios
