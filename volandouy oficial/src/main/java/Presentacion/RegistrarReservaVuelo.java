...
/**
 * JInternalFrame para registrar una reserva de vuelo.
 */
...
public class RegistrarReservaVuelo extends JInternalFrame {
  ...
  private final JComboBox<TipoAsiento> comboTipoAsiento = new JComboBox<>(TipoAsiento.values()); // usar
                                                                                                 // enum
                                                                                                 // directo

  private final JComboBox<Equipaje> comboEquipaje = new JComboBox<>(Equipaje.values()); // usar enum
                                                                                        // directo
  ...
  // --------- Ctor ---------
  /**
   * Crea el JInternalFrame para registrar una reserva de vuelo.
   *
   * 
   */
  ...
  private void armarUI() {
    ...
    // Selección de vuelo
    JLabel labelAerolinea = new JLabel("Aerolínea:");
    labelAerolinea.setFont(new Font("Comic Sans MS", Font.PLAIN, 11));
    form.add(labelAerolinea);
    comboAerolinea.setFont(new Font("Comic Sans MS", Font.PLAIN, 11));
    form.add(comboAerolinea);

    JLabel labelRuta = new JLabel("Ruta:");
    labelRuta.setFont(new Font("Comic Sans MS", Font.PLAIN, 11));
    form.add(labelRuta);
    comboRuta.setFont(new Font("Comic Sans MS", Font.PLAIN, 11));
    form.add(comboRuta);

    JLabel labelVuelo = new JLabel("Vuelo:");
    labelVuelo.setFont(new Font("Comic Sans MS", Font.PLAIN, 11));
    form.add(labelVuelo);
    ...
    labelCliente.setFont(new Font("Comic Sans MS", Font.PLAIN, 11));
    form.add(labelCliente);
    ...
    labelFechaReserva.setFont(new Font("Comic Sans MS", Font.PLAIN, 11));
    form.add(labelFechaReserva);
    dateChooser.setDate(new Date());
    dateChooser.setDateFormatString("yyyy-MM-dd");
    form.add(dateChooser);

    JLabel labelTipoAsiento = new JLabel("Tipo de asiento:");
    labelTipoAsiento.setFont(new Font("Comic Sans MS", Font.PLAIN, 11));
    form.add(labelTipoAsiento);
    comboTipoAsiento.setFont(new Font("Comic Sans MS", Font.PLAIN, 11));
    form.add(comboTipoAsiento);

    JLabel labelTipoEquipaje = new JLabel("Tipo de equipaje:");
    labelTipoEquipaje.setFont(new Font("Comic Sans MS", Font.PLAIN, 11));
    form.add(labelTipoEquipaje);
    comboEquipaje.setFont(new Font("Comic Sans MS", Font.PLAIN, 11));
    form.add(comboEquipaje);

    JLabel labelCantidadPasajes = new JLabel("Cantidad de pasajes:");
    labelCantidadPasajes.setFont(new Font("Comic Sans MS", Font.PLAIN, 11));
    form.add(labelCantidadPasajes);
    spinnerPasajes.setFont(new Font("Comic Sans MS", Font.PLAIN, 11));
    form.add(spinnerPasajes);

    JLabel labelEquipajeExtra = new JLabel("Equipaje extra (unid.):");
    labelEquipajeExtra.setFont(new Font("Comic Sans MS", Font.PLAIN, 11));
    form.add(labelEquipajeExtra);
  ...
  private void cargarClientes() {
    ...
    System.out
        .println("Cantidad de clientes encontrados: " + (clientes != null ? clientes.size() : 0));
    if (clientes != null) {
  ...
  private void registrar() {
    ...
    if (a == null || r == null || v == null || dc == null || fecha == null) {
      JOptionPane.showMessageDialog(this, "Seleccioná Aerolínea, Ruta, Vuelo, Cliente y Fecha.",
    ...
    java.util.List<DataReserva> reservasExistentes = sistema.listarReservas(a.getNickname(),
        r.getNombre(), v.getNombre());
    ...
    if (pasajesReservados + cantPasajes > cuposDisponibles) {
      JOptionPane.showMessageDialog(this,
          "No hay cupos suficientes para este tipo de asiento. Disponibles: "
              + (cuposDisponibles - pasajesReservados),
    ...
    try {
      ...
      if (ruta != null && tipoAsiento != null) {
        ...
        java.math.BigDecimal precioAsiento = (tipoAsiento == TipoAsiento.EJECUTIVO ? precioEje
            : precioTur);
        if (precioAsiento != null) {
          java.math.BigDecimal subAsientos = precioAsiento
              .multiply(java.math.BigDecimal.valueOf(cantPasajes));
          java.math.BigDecimal subExtra = unitExtra
              .multiply(java.math.BigDecimal.valueOf(equipajeExtra));
      ...
      DataReserva datos = new DataReserva(0, // idReserva (lo asigna backend)
          fecha, tipoAsiento, equipaje, equipajeExtra, costoTotal, // costo total calculado
          dc);
      ...
      // 2) pedir pasajeros
      PasajerosDialog dlg = new PasajerosDialog(SwingUtilities.getWindowAncestor(this), cantPasajes,
      ...
      sistema.registrarReserva(a.getNickname(), r.getNombre(), v.getNombre(), // si usás código
                                                                              // distinto, cambialo
          datos);
  ...
  private static final class PasajerosDialog extends JDialog {
    ...
    PasajerosDialog(Window owner, int cantidad, DataCliente cliente) {
      ...
      String nomCliente = (cliente != null && cliente.getNombre() != null)
          ? cliente.getNombre().trim()
          : "";
      String apeCliente = (cliente != null && cliente.getApellido() != null)
          ? cliente.getApellido().trim()
          : "";
      if (cantidad > 0) {
        data[0][0] = nomCliente;
        data[0][1] = apeCliente;
      ...
      javax.swing.table.DefaultTableModel model = new javax.swing.table.DefaultTableModel(data,
          cols) {
        @Override
      ...
      btnOk.addActionListener(e -> {
        ...
        if (table.isEditing()) {
          ...
          if (ed != null && !ed.stopCellEditing()) {
            return; // si el editor no puede cerrar, aborta
          }
        }
        ...
        for (int i = 0; i < model.getRowCount(); i++) {
          ...
          if (nom.isEmpty() || ape.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Completá nombre y apellido en la fila " + (i + 1),
  ...
  private String formatearVuelo(Object v) {
    ...
    if (!cupos.isEmpty()) {
      sb.append("  Cupos:  ").append(cupos).append("\n");
    }
    if (!avion.isEmpty()) {
      sb.append("  Avión:  ").append(avion).append("\n");
    }
  ...
  private String formatearRuta(Object r) {
    ...
    if (!desc.isEmpty()) {
      sb.append("  Descr.:   ").append(desc).append("\n");
    }
    if (!origen.isEmpty()) {
    ...
    if (!cTur.isEmpty()) {
      sb.append("  Turista:  ").append(cTur).append("\n");
    }
    if (!cEje.isEmpty()) {
      sb.append("  Ejecutivo:").append(cEje).append("\n");
    }
    if (!equipEx.isEmpty()) {
      sb.append("  Equipaje extra (U): ").append(equipEx).append("\n");
    }
  ...
  private static String ciudad(Object c) {
    ...
    if (!nom.isBlank() || !pais.isBlank()) {
      return (nom + (pais.isBlank() ? "" : ", " + pais)).trim();
    }
    return c.toString();
  }
  ...
  private String calcularCostoEstimado(DataRuta r, TipoAsiento tipo, int cantPasajes,
      int unidadesExtra) {
    ...
    java.math.BigDecimal precioAsiento = (tipo == TipoAsiento.EJECUTIVO ? precioEje : precioTur);

    if (precioAsiento == null) {
      return ""; // sin base no mostramos costo
    }
    ...
    sb.append("\n  Equipaje extra: ").append(unidadesExtra).append("  x ").append(money(unitExtra))
