...
public class RegistroRutaVueloaPaquete extends JInternalFrame {
    ...
    */
    ...
    private void cargarPaquetes() {
        ...
        try {
            for (DataPaquete p : sistema.listarPaquetesSinCompras()) {
                comboBoxPaquetes.addItem(new Item<>(p.getNombre(), p.getNombre() + " — " + p.getDescripcion()));
            }
        } catch (Exception ex) {
    ...
    private void onGuardar() {
        ...
        // Validaciones UI
        if (pSel == null) { warn("Debe seleccionar un paquete."); return; }
        if (aSel == null) { warn("Debe seleccionar una aerolínea."); return; }
        if (rSel == null) { warn("Debe seleccionar una ruta."); return; }
        if (tipo == null) { warn("Debe seleccionar un tipo de asiento."); return; }
        if (cupos <= 0)    { warn("La cantidad de cupos debe ser mayor a cero."); return; }

        try {
            // Llamar a la lógica
            sistema.agregarRutaAPaquete(
                    pSel.value(),       // nombre del paquete
                    aSel.value(),       // nickname de aerolínea
                    rSel.value(),       // nombre de la ruta
                    tipo,
                    cupos               // cantidad de cupos
            );
    ...
    private static final class Item<T> {
        ...
        Item(T value, String display) { this.value = value; this.display = display; }
