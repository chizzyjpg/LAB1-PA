...
/**
 * Clase para aceptar o rechazar rutas de vuelo ingresadas por aerolíneas.
 */
public class AceptarRechazarRuta extends JInternalFrame {
  ...
  private final JLabel lblEstado = new JLabel(
      "Seleccione una aerolínea y presione “Listar rutas ingresadas”.", JLabel.LEFT);
  ...
  private void onListar(ActionEvent e) {
    ...
    if (aero == null || aero.isBlank()) {
      JOptionPane.showMessageDialog(this, "Seleccione una aerolínea.", "Validación",
    ...
    try {
      ...
      if (rutas != null) {
        for (Logica.DataRuta r : rutas) {
          // Mostrar solo las INGRESADAS (si viene null por datos viejos, las tratamos
          // como INGRESADAS)
      ...
      lblEstado
          .setText(count == 0 ? "No hay rutas en estado “Ingresada” para la aerolínea seleccionada."
      ...
      JOptionPane.showMessageDialog(this, "Error al listar rutas: " + ex.getMessage(), "Error",
  ...
  private void onCambiarEstado(EstadoRuta nuevo) {
    ...
    if (row < 0) {
      JOptionPane.showMessageDialog(this, "Seleccione una ruta.", "Validación",
          JOptionPane.WARNING_MESSAGE);
      return;
    }
    ...
    int ok = JOptionPane.showOptionDialog(this,
        ...
        "Confirmación", JOptionPane.OK_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE, null, opciones,
        opciones[0]);
    ...
    try {
      ...
      // Como esta vista muestra "Ingresadas", quitamos la fila (ya no debería
  ...
  private void cargarAerolineasDesdeSistema() {
    try {
      ...
      if (cboAerolinea.getItemCount() > 0) {
        cboAerolinea.setSelectedIndex(0);        
      }
    } catch (Exception ex) {
      JOptionPane.showMessageDialog(this,
          "No se pudieron cargar las aerolíneas: " + ex.getMessage(), "Error",
          JOptionPane.ERROR_MESSAGE);
    }
  }
  ...
  private void actualizarLblEstado() {
    ...
    lblEstado.setText(vacia ? "No hay rutas en estado “Ingresada” para la aerolínea seleccionada."
        : "Seleccione una ruta y confirme o rechace.");
  }
