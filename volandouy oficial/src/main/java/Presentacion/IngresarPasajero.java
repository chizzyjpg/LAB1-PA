...
/**
 * Clase que representa la interfaz gráfica para ingresar un nuevo pasajero.
 */
public class IngresarPasajero extends JInternalFrame {
   ...
   * Runnable() { public void run() { try { IngresarPasajero frame = new
   * IngresarPasajero(); frame.setVisible(true); } catch (Exception e) {
   * e.printStackTrace(); } } }); }
   */
  ...
  public IngresarPasajero(ISistema sistema) {
    ...
    btnGuardar.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        ...
        String sexo = rdbtnFemenino.isSelected() ? "Femenino"
            : rdbtnMasculino.isSelected() ? "Masculino" : "";

        // Validaciones
        if (nombre.isEmpty()) {
          JOptionPane.showMessageDialog(IngresarPasajero.this, "El nombre no puede estar vacío.",
              "Error", JOptionPane.ERROR_MESSAGE);
        }

        if (apellido.isEmpty()) {
          JOptionPane.showMessageDialog(IngresarPasajero.this, "El apellido no puede estar vacío.",
              "Error", JOptionPane.ERROR_MESSAGE);
        }

        if (sexo.isEmpty()) {
          JOptionPane.showMessageDialog(IngresarPasajero.this, "Debe seleccionar un sexo.", "Error",
