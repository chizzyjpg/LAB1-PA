...
/**
 * Clase que representa la interfaz gráfica para registrar un nuevo paquete de rutas de vuelo.
 */
public class RegistroPaqueteRuta extends JInternalFrame {
  ...
  /** Constructor de la clase RegistroPaqueteRuta.
   * 
   */
  public RegistroPaqueteRuta(ISistema sistema) {
    ...
    getContentPane().setLayout(
        new MigLayout("insets 12, fillx, gap 10", "[right][grow]", "[][][grow][][::40][]"));
  ...
  private void construirFormulario() {
    ...
    JScrollPane spDesc = new JScrollPane(taDescripcion, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
  ...
  private void onAceptar() {
    try {
      ...
      if (desc.isBlank()) {
        warn("La descripción es obligatoria.");
        taDescripcion.requestFocus();
        return;
      }
      if (fecha == null) {
        warn("La fecha de alta es obligatoria.");
        dcFechaAlta.requestFocus();
        return;
      }
      if (validez <= 0) {
        warn("La validez debe ser mayor a 0.");
        spValidez.requestFocus();
        return;
      }
      if (dto < 0 || dto > 100) {
        warn("El descuento debe estar entre 0 y 100.");
        spDescuento.requestFocus();
        return;
      }
      ...
      JOptionPane.showMessageDialog(this, "Paquete creado correctamente.", "OK",
