...
/**
 * Clase que representa la interfaz gráfica para consultar paquetes de rutas de vuelo.
 */
public class ConsultaPaqRutasVuelo extends JInternalFrame {
  ...
  private JTextField txtCantRutas;
  ...
  /** Constructor de la clase ConsultaPaqRutasVuelo.
   * 
   */
  public ConsultaPaqRutasVuelo(ISistema sistema) {
    ...
    /* ----------- Datos del paquete ----------- */
    int y = 60;
    txtNombre = roField("Nombre:", 10, y, 180, 28, 260);
    y += 34;
    txtCosto = roField("Costo:", 10, y, 180, 28, 160);
    txtValidez = roField("Validez (días):", 380, y, 150, 28, 120);
    y += 34;
  ...
  private void cargarPaquetes() {
    ...
    try {
      ...
      if (lista != null) {
        lista.stream().sorted((a, b) -> a.getNombre().compareToIgnoreCase(b.getNombre()))
  ...
  private void mostrarDetalleSeleccionado() {
    ...
    if (d == null) {
      JOptionPane.showMessageDialog(this, "Debe seleccionar un paquete de rutas de vuelo.", "Error",
    ...
    if (rutas != null && !rutas.isEmpty()) {
      rutas.stream().sorted(String.CASE_INSENSITIVE_ORDER)
  ...
  private void abrirConsultaRutaVuelo() {
    ...
    if (row == -1) {
      JOptionPane.showMessageDialog(this, "Seleccioná una ruta en la tabla antes de continuar.",
    ...
    try {
      ConsultaRutaVuelo frame = new ConsultaRutaVuelo(sistema); // ajustá si tenés sobrecarga con
                                                                // nombreRuta
      dp.add(frame);
