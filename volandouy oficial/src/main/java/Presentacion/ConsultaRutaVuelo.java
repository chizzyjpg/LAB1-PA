...
/**
 * Clase que representa la interfaz gráfica para consultar rutas de vuelo y sus vuelos
 * asociados.
 */
public class ConsultaRutaVuelo extends JInternalFrame {
  ...
  /**
   * Constructor de la clase ConsultaRutaVuelo.
   * 
   */
  public ConsultaRutaVuelo(ISistema sistema) {
    ...
    right.add(flightsPanel, BorderLayout.SOUTH); // (podés cambiarlo por CENTER si preferís que
                                                 // ocupe más alto)
    ...
    listRutas.addListSelectionListener(e -> {
      if (!e.getValueIsAdjusting()) {
        mostrarDetalleRutaYVuelos((DataAerolinea) comboAerolineas.getSelectedItem(),
            listRutas.getSelectedValue());
      }
    });
  ...
  private void cargarAerolineas() {
    ...
    try {
      List<DataAerolinea> aas = sistema.listarAerolineas(); // <-- ajustá si tu método se llama
                                                            // distinto
  ...
  private void cargarRutasDeSeleccion() {
    ...
    try {
      List<DataRuta> rutas = sistema.listarPorAerolinea(sel.getNickname()); // <-- implementa esto
                                                                            // en tu manejador
  ...
  private void verVueloSeleccionado() {
    ...
    if (v == null) {
      JOptionPane.showMessageDialog(this, "Seleccioná un vuelo", "Atención",
    ...
    if (a == null || r == null) {
      JOptionPane.showMessageDialog(this, "Falta aerolínea o ruta seleccionada.", "Atención",
  ...
  private String formatRuta(DataRuta r) {
    ...
    return """
        ...
        Costo Ejecutivo: %s
        """.formatted(r.getNombre(), dc, nullToDash(r.getDescripcion()), o, d, h, f, ct, ce, cj);
  }
