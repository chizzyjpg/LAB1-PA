...
/**
 * Clase que representa la interfaz gráfica para el registro de vuelos.
 */
public class RegistroVuelo extends JInternalFrame {
  ...
  /**
   * Constructor de la clase RegistroVuelo.
   *
   * 
   * */
  public RegistroVuelo(ISistema sistema) {
    ...
    for (DataRuta da : sistema
        .listarPorAerolinea(((DataAerolinea) comboAerolinea.getSelectedItem()).getNickname())) {
    ...
    btnNewButtonAceptar.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        if (textNombreVuelo.getText().trim().isEmpty() || textDuracion.getText().trim().isEmpty()
            || textCantTurista.getText().trim().isEmpty()
            || textMaxEjecutivo.getText().trim().isEmpty()) {
          JOptionPane.showMessageDialog(null, "Debe completar todos los campos.", "Error",
        ...
        if (aerolineaSeleccionada == null) {
          JOptionPane.showMessageDialog(null, "Debe seleccionar una aerolínea.", "Error",
              JOptionPane.ERROR_MESSAGE);
          return;
        }
        if (rutaSeleccionada == null) {
          JOptionPane.showMessageDialog(null, "Debe seleccionar una ruta.", "Error",
              JOptionPane.ERROR_MESSAGE);
          return;
        }
        if (dateChooserFechaVuelo.getDate() == null || dateChooserFechaAlta.getDate() == null) {
          JOptionPane.showMessageDialog(null, "Debe seleccionar ambas fechas.", "Error",
              JOptionPane.ERROR_MESSAGE);
          return;
        }
        ...
        if (fecha.before(startOfToday())) {
          JOptionPane.showMessageDialog(RegistroVuelo.this, "La fecha no puede ser anterior a hoy.",
        ...
        try {
          ...
          DataVueloEspecifico vueloData = new DataVueloEspecifico(textNombreVuelo.getText().trim(),
              dateChooserFechaVuelo.getDate(), duracion, cantTurista, maxEjecutivo,
              dateChooserFechaAlta.getDate(), rutaSeleccionada);

          sistema.registrarVuelo(aerolineaSeleccionada.getNickname(), rutaSeleccionada.getNombre(),
              vueloData);

          JOptionPane.showMessageDialog(null, "¡Vuelo registrado con éxito!", "Éxito",
